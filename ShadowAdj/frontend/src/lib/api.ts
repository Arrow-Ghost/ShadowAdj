...
export async function fetchCoaching(id: string): Promise<{
  notes: string;
  structured?: any;
  experiment?: { title: string; goal: string; action: string };
  at: number;
}> {
...
export async function deleteSession(id: string): Promise<{ ok: boolean; id: string }> {
  const r = await fetch(`${API_BASE}/api/sessions/${id}`, { method: 'DELETE' });
  if (!r.ok) throw new Error('could not delete session');
  return r.json();
}

export async function fetchConsent(id: string): Promise<any> {
  const r = await fetch(`${API_BASE}/api/sessions/${id}/consent`);
  if (!r.ok) throw new Error('could not fetch consent record');
  return r.json();
}
